	...
	if (ReInfo->s->_raceType == RM_TYPE_QUALIF) {
		...
		if (!strcmp(gridType, RM_VAL_LAST_RACE_ORDER)) {
			...
			for (i = 1; i < nCars + 1; i++) {
				...
				if (i==4 && !TrafficBool) GfParmSetStr(params, path2, RM_ATTR_MODULE, "inferno"); // in section "Drivers Start List/i"  set "module" to (result of value "module" in subsection 1 of section 1
